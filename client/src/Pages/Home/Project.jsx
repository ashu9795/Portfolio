...
function Project() {
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row py-10 lg:gap-20 gap-10">
        ...
        <div className="flex sm:flex-col gap-5 border-l-2 border-gray-400 w-full sm:w-full overflow-x-auto ">
  {projects.map((project, index) => (
    <div
      key={index}
      onClick={() => setSelectedItemIndex(index)}
      className="cursor-pointer " // Ensure the width is based on content, no shrinking
    >
      <h1
        className={`text-lg sm:text-xl px-3 sm:px-5 ${
  ...
  <img src={projects[selectedItemIndex].image} alt=""  className="h-40 w-48 sm:h-60 sm:w-72 "  />
          ...
          <h1 className="text-orange-200 text-xl sm:text-2xl">
            {projects[selectedItemIndex].technologies}
          </h1>
          <h1 className="text-green-200 text-lg sm:text-2xl">
